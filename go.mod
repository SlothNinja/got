module github.com/SlothNinja/got

require (
	cloud.google.com/go/datastore v1.1.0
	github.com/SlothNinja/codec v0.0.0-20200229214548-e3da7deddffb
	github.com/SlothNinja/color v0.0.0-20200316011031-586fe3c95fae
	github.com/SlothNinja/contest v0.0.0-20200316005344-c79242ea0e22
	github.com/SlothNinja/game v0.0.0-20200316005656-3f839025ac43
	github.com/SlothNinja/log v0.0.2
	github.com/SlothNinja/mlog v0.0.0-20200316010411-7a902d317278
	github.com/SlothNinja/rating v0.0.0-20200316005149-e563eb99a046
	github.com/SlothNinja/restful v0.0.0-20200316004945-5dc79423e483
	github.com/SlothNinja/schema v0.0.0-20200301053948-c765147c924d
	github.com/SlothNinja/send v0.0.0-20200316010642-9eccb6544327
	github.com/SlothNinja/sn v0.0.0-20200229185633-ae23f40d2db2
	github.com/SlothNinja/type v0.0.0-20200316011604-5ac825cb08e3
	github.com/SlothNinja/user v0.0.0-20200316001319-fa6de0ac8141
	github.com/SlothNinja/user-stats v0.0.0-20200316003629-dfe63e057543
	github.com/gin-gonic/gin v1.5.0
	google.golang.org/appengine v1.6.5
)
